...
const metricConfig = {
    ...
    temp: { getValue: m => m.temperature?.current, getClass: v => v > 80 ? 'danger' : v > 70 ? 'warning' : '', format: v => v.toFixed(1) + '°C', chartKey: 'temperature' },
...
function renderMiniChart(canvasId, data, label, colorKey, valueKey) {
    ...
    const chartData = data.map(e => ({ x: e.timestamp * 1000, y: e[valueKey] })).filter(d => d.y !== null);
    const color = CHART_COLORS[colorKey] || CHART_COLORS.purple;
    const formatters = { cpu_usage: v => v.toFixed(1)+'%', free_mb: v => v.toFixed(0)+' MB', temperature: v => v.toFixed(1)+'°C', signal_dbm: v => v.toFixed(0)+' dBm', avg_latency: v => v.toFixed(1)+' ms' };
                ...
                if (metrics.online) {
                    [['cpu', 'red', 'cpu_usage'], ['memory', 'purple', 'free_mb'], ['temp', 'orange', 'temperature'], ['wireless', 'teal', 'signal_dbm'], ['ping', 'indigo', 'avg_latency']]
                        .forEach(([key, color, field]) => {
                            ...
                            const label = key === 'cpu' ? 'CPU %' : key === 'memory' ? 'Memory MB' : key === 'temp' ? 'Temp °C' : key === 'wireless' ? 'Signal dBm' : 'Latency ms';
