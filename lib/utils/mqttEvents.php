 ...
 */
define('MQTT_EVENT_SEQ_START', 'ss');
define('MQTT_EVENT_SEQ_STOP', 'se');
define('MQTT_EVENT_PL_START', 'ps');
define('MQTT_EVENT_PL_STOP', 'pe');
define('MQTT_EVENT_STATUS', 'st');
define('MQTT_EVENT_MEDIA_START', 'ms');
define('MQTT_EVENT_MEDIA_STOP', 'me');
define('MQTT_EVENT_WARNING', 'wn');

/**
 * Map short event codes to human-readable labels
 */
function getMqttEventLabel($eventType) {
    $labels = [
        MQTT_EVENT_SEQ_START => 'Sequence Start',
        MQTT_EVENT_SEQ_STOP => 'Sequence Stop',
        MQTT_EVENT_PL_START => 'Playlist Start',
        MQTT_EVENT_PL_STOP => 'Playlist Stop',
        MQTT_EVENT_STATUS => 'Status Update',
        MQTT_EVENT_MEDIA_START => 'Media Start',
        MQTT_EVENT_MEDIA_STOP => 'Media Stop',
        MQTT_EVENT_WARNING => 'Warning'
    ];
 ...
 * @return bool Success
 */
function writeMqttEvent($hostname, $eventType, $data, $duration = null) {
    $entry = [
        't' => time(),
        'h' => $hostname,
        'e' => $eventType,
    ...
    $metricsFile = WATCHERMQTTEVENTSFILE;
    $fp = @fopen($metricsFile, 'a');
    if (!$fp) {
        logMessage("ERROR: Unable to open MQTT events file for writing");
        return false;
    }
 ...
 */
function getMqttEvents($hoursBack = 24, $hostname = null, $eventType = null) {
    $metricsFile = WATCHERMQTTEVENTSFILE;
            ...
            if (preg_match('/\[.*?\]\s+(.+)$/', $line, $matches)) {
                ...
                if ($entry && isset($entry['t'])) {
                    ...
                    $eventData = [
                        ...
                        'eventLabel' => getMqttEventLabel($entry['e'] ?? ''),
                        'data' => $entry['d'] ?? ''
                    ];
 ...
 */
function getMqttEventStats($hoursBack = 24) {
    $result = getMqttEvents($hoursBack);
    ...
    foreach ($events as $event) {
        ...
        // Track sequences
        if ($type === MQTT_EVENT_SEQ_START && !empty($event['data'])) {
        ...
        // Track playlists
        if ($type === MQTT_EVENT_PL_START && !empty($event['data'])) {
        ...
        // Track media (songs)
        if ($type === MQTT_EVENT_MEDIA_START && !empty($event['data'])) {
        ...
        if ($type === MQTT_EVENT_WARNING && !empty($event['data'])) {
            $stats['warnings'][] = [
                'time' => $event['datetime'],
                'host' => $event['hostname'],
                'message' => $event['data']
 ...
 */
function getMqttHostsList() {
    $result = getMqttEvents(0); // All time
 ...
 *
 * @param int $retentionDays Number of days to retain (from config)
 */
function rotateMqttEventsFile($retentionDays = 60) {
    $metricsFile = WATCHERMQTTEVENTSFILE;
    ...
    if ($tempFp) {
        ...
        logMessage("MQTT events purge: removed {$purgedCount} old entries, kept " . count($recentEvents) . " recent entries.");

        ensureFppOwnership($metricsFile);
        ensureFppOwnership($backupFile);
    } else {
        logMessage("ERROR: Unable to create temp file for MQTT events purge");
    }
